...
-- Представляю Вашему вниманию БД посвященную программе лояльности клиентов в рамках сети магазнинов розничной торговли электронной техникой.
...
CREATE TABLE profiles (
  ...
  card_id INT UNSIGNED NOT NULL UNIQUE COMMENT "Идентификатор номера бонусной карты",
...
store_id INT UNSIGNED NOT NULL COMMENT "Идентификатор магазина выдачи карты",
created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT "Время создания строки",  
updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT "Время обновления строки"
) COMMENT "Карты"; 
...
card_id INT UNSIGNED NOT NULL COMMENT "Идентификатор номера бонусной карты",
store_id INT UNSIGNED NOT NULL COMMENT "Идентификатор номера магазина покупки",
...
cheque_id INT UNSIGNED NOT NULL UNIQUE COMMENT "Идентификатор номера чека покупки",
created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT "Время создания строки",  
updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT "Время обновления строки"
...
ALTER TABLE cards ADD CONSTRAINT cards_store_id_fk FOREIGN KEY (store_id) REFERENCES stores(store_id);
ALTER TABLE contacts ADD CONSTRAINT contacts_user_id_fk FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE CASCADE;
ALTER TABLE profiles ADD CONSTRAINT profiles_card_id_fk FOREIGN KEY (card_id) REFERENCES cards(card_id);
ALTER TABLE transactions ADD CONSTRAINT transactions_card_id_fk FOREIGN KEY (card_id) REFERENCES cards(card_id);
ALTER TABLE transactions ADD CONSTRAINT transactions_store_id_fk FOREIGN KEY (store_id) REFERENCES stores(store_id);
ALTER TABLE transactions ADD CONSTRAINT transactions_cheque_id_fk FOREIGN KEY (cheque_id) REFERENCES cheque(cheque_id);
...
INSERT INTO cards (
  card_num, card_status, card_type, store_id)
SELECT
  cards_numbers,
...
UPDATE profiles as p left join cards as c on p.card_id = c.card_id SET active = 0 where c.card_status = 5; -- У неактивированных карт в поле 'Активный счёт' прописываем  - 0
UPDATE profiles as p left join cards as c on p.card_id = c.card_id SET reserve = 0 where c.card_status = 5; -- У неактивированных карт в поле 'Резервный счёт' прописываем  - 0
...
card_id, store_id, write_off, accrual, cheque_id)
SELECT
(select c from (select card_id as c from profiles where active > 0) as n ORDER BY rand() LIMIT 1), -- Не создаём транзакций по неактивированным картам
(select store_id FROM stores ORDER BY rand() LIMIT 1),
(select active_write_off FROM random_data ORDER BY rand() LIMIT 1),
(select reserve_accrual FROM random_data ORDER BY rand() LIMIT 1),
cheque_id from cheque;
...
select s.store_num, c.purchase, count(c.purchase) from cheque c left join transactions t on t.cheque_id = c.cheque_id left join stores s on t.store_id = s.store_id group by c.purchase, t.store_id order by store_num;
...
select count(t.card_id) as total, c.card_num, u.first_name, u.birthday, co.email, co.phone from transactions t 
left join profiles p on p.card_id = t.card_id 
left join users u on u.user_id = p.user_id 
left join contacts co on co.user_id = u.user_id
left join cards c on t.card_id = c.card_id 
group by t.card_id order by total desc limit 10;


-- Узнаем общее количество списанных и начисленных бонусных баллов в разрезе магазинов.
select s.store_num, sum(write_off), sum(accrual) from transactions t left join stores s on s.store_id = t.store_id group by store_num;


-- Количество выданных карт лояльностив разрезе магазинов
select count(card_num), store_num from cards c left join stores s on s.store_id = c.store_id group by c.store_id order by count(card_num) desc;
...
SELECT c.card_num, cc.description, s.store_num
FROM cards c left join card_types cc on c.card_type = cc.type_id left join stores s on s.store_id = c.store_id
WHERE card_status = 1;
...
select cr.card_num, p.active, u.first_name, c.phone from profiles p left join users u on u.user_id = p.user_id 
left join contacts c on c.user_id = u.user_id left join cards cr on cr.card_id = p.card_id order by active desc limit 5;
...
-- Реализуем процедуру, в которой будем использовать view bonuses, чтобы разослать sms: 
-- cоздадим таблицу, в которую будем добавлять текст sms и отслеживать признак отправки. 


DROP TABLE IF EXISTS sms;
CREATE TABLE sms (phone varchar(100), text varchar(100), is_sent boolean);

DELIMITER //
DROP PROCEDURE IF EXISTS send_sms//
CREATE PROCEDURE send_sms()
BEGIN
DECLARE finished INT DEFAULT 0;
DECLARE name varchar(100) DEFAULT "";
DECLARE bonuses varchar(100) DEFAULT "";
DECLARE phone varchar(100) DEFAULT "";

DEClARE cur CURSOR FOR SELECT имя, бонусы, телефон FROM bonuses;
...
getUsers: LOOP
    FETCH cur INTO name, bonuses, phone;
    IF finished = 1 THEN LEAVE getUsers; END IF;
    INSERT INTO sms (phone, text, is_sent) VALUES (phone,
      concat('Добрый день, ', name, '!\n',
      'У Вас на счете ', bonuses, ' бонусов'),
      false
    );
END LOOP getUsers;
CLOSE cur;
